...
print('-'*30)
# retorna um objeto pelo id (se não existir, retorna None)
...
print('-'*30)
d = session.query(Funcionario).filter(Funcionario.salario >
                                      1500).order_by(Funcionario.nome).all()
for i in d:
    print(i.id, i.nome, i.idade, i.salario)

# Busca utilizando filtros (retorna todos)
print('-'*30)
d = session.query(Funcionario).filter(Funcionario.idade ==
                                      22, Funcionario.nome.like('%inho%')).all()
for i in d:
    print(i.id, i.nome, i.idade, i.salario)
